...
class NTUDataLoaders(object):
    ...
    def torgb(self, ske_joints):
        ...
        for ske_joint in ske_joints:
            ...
            rgb_ske = np.reshape(
                ske_joint, (ske_joint.shape[0], ske_joint.shape[1] // 3, 3))
            rgb_ske = scipy.misc.imresize(
                rgb_ske, (224, 224)).astype(np.float32)
...
def padding(joints, max_len=300, pad_value=0.):
    ...
    if feat_dim == 75:
        joints = np.hstack(
            (joints, np.zeros((num_frames, 75), dtype=joints.dtype)))
